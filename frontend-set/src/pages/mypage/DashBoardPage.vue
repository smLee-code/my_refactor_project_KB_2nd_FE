            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <p class="text-sm text-gray-600 mb-5">
                            직접 생성한 챌린지 목록을 확인하고 참여자 인증을 관리합니다.
                        </p>
                        <router-link
                            to="/admin/my-challenges"
                            class="w-full text-center bg-green-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-green-700 transition-colors duration-300 block"
                        >
                            관리 페이지로 이동
                        </router-link>
                    </div>
...
onMounted(async () => {
    ...
    try {
        ...
        if (res.data && res.data.length > 0) {
            ...
            fundingStats.value.progressFunding = allFundings.filter(
                (f) => f.progress === 'Launch',
            ).length
