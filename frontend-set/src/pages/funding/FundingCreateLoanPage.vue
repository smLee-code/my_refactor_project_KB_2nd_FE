      ...
      <form class="space-y-8">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">
            대출 상품명 <span class="text-red-500">*</span>
          </label>
          <input
            type="text"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              최소 금리 <span class="text-red-500">*</span>
            </label>
            <div class="relative">
              <input
                type="number"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              최대 금리 <span class="text-red-500">*</span>
            </label>
            <div class="relative">
              <input
                type="number"
        ...
        <div>
          ...
          <input
            ...
            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
            placeholder="예: 만 20-39세, 사업자등록증 필요"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="대출 상품에 대한 상세한 설명을 입력해주세요"
          ></textarea>
        </div>
        <!-- 리워드 설정 -->
        <div>
          <h3 class="text-lg font-semibold text-gray-900 mb-4">리워드 및 혜택</h3>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
                리워드 내용
...
const submitFunding = async () => {
  try {
    // 유효성 검사
    if (!formData.value.name || !formData.value.detail || !formData.value.launchDate || !formData.value.endDate) {
    ...
    const jsonData = {
      ...
      loanLimit: parseInt(String(formData.value.loanLimit).replace(/,/g, '')) || 0,
      minInterestRate: parseFloat(formData.value.minInterestRate) || 0,
      maxInterestRate: parseFloat(formData.value.maxInterestRate) || 0,
      ...
      financialInstitution: '신한은행', // 임시값 고정
      keywordIds: [11, 12, 13]
    }
    ...
    const response = await createLoanFunding(formDataToSend)
    alert('펀딩 프로젝트가 성공적으로 생성되었습니다.')
