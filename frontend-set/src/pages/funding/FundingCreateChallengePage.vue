...
const submitFunding = async () => {
  try {
    // 유효성 검사
    if (!formData.value.name || !formData.value.detail || !formData.value.launchDate || !formData.value.endDate || !formData.value.joinCondition || !formData.value.challengePeriodDays || !formData.value.reward || !formData.value.rewardCondition || !formData.value.verifyStandard) {
    ...
    alert('챌린지 펀딩 프로젝트가 성공적으로 생성되었습니다.')
