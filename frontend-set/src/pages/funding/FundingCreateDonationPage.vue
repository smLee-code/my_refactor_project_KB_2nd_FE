...
const submitFunding = async () => {
    try {
        ...
        if (
            ...
            !formData.value.usagePlan ||
            !formData.value.minDonationAmount ||
            !formData.value.maxDonationAmount ||
        ...
        const jsonData = {
            ...
            minDonationAmount:
                parseInt(String(formData.value.minDonationAmount).replace(/,/g, '')) || 0,
            maxDonationAmount:
                parseInt(String(formData.value.maxDonationAmount).replace(/,/g, '')) || 0,
            targetAmount: parseInt(String(formData.value.targetAmount).replace(/,/g, '')) || 0,
    ...
    if (projectId) {
        try {
            ...
            if (projectData.detailInfo) {
                formData.value.targetAmount = projectData.detailInfo.targetAmount
                    ? projectData.detailInfo.targetAmount.toLocaleString()
                    : ''
                // recipient 또는 recipientOrganization 필드 확인
                formData.value.recipient =
                    projectData.detailInfo.recipient ||
                    projectData.detailInfo.recipientOrganization ||
                    ''
                formData.value.usagePlan =
                    projectData.detailInfo.usagePlan || projectData.detailInfo.donationUsePlan || ''

                // 기부 금액 범위 설정
                if (projectData.detailInfo.minDonationAmount) {
                    formData.value.minDonationAmount =
                        projectData.detailInfo.minDonationAmount.toLocaleString()
                }
                if (projectData.detailInfo.maxDonationAmount) {
                    formData.value.maxDonationAmount =
                        projectData.detailInfo.maxDonationAmount.toLocaleString()
                }

                // 가입 조건 설정
                formData.value.joinCondition =
                    projectData.detailInfo.joinCondition || '누구나 참여 가능'
            }

            // 키워드 정보가 있다면 추가
            if (projectData.basicInfo.keywords) {
                formData.value.keywordIds = projectData.basicInfo.keywords.map(
                    (keyword) => keyword.id,
                )
            }
